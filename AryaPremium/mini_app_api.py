...
async def verify_payment(payload: dict):
    ...
    # Send Logs to Channel if configured
    try:
        from config import Config
        if Config.PAYMENT_LOGS_CHANNEL:
            from plugins.userbot.market_seller import market_clients
            # Get the first available premium bot client to send the log
            # Or use the main bot if the DB function supports it. 
            # In AryaPremium, add_purchase doesn't log automatically, so let's log it here.
            bot_client = next(iter(market_clients.values())) if market_clients else None
            if bot_client:
                log_text = (
                    f"✅ <b>NEW RAZORPAY PAYMENT (MINI APP)</b>\n\n"
                    f"<b>User:</b> {username} [<code>{tg_id}</code>]\n"
                    f"<b>Stories:</b> {', '.join([s.get('story_name_en', '') for s in stories])}\n"
                    f"<b>Amount:</b> ₹{total}\n"
                    f"<b>Payment ID:</b> <code>{rzp_payment_id}</code>\n"
                    f"<b>Order ID:</b> <code>{oid}</code>"
                )
                import asyncio
                asyncio.create_task(bot_client.send_message(Config.PAYMENT_LOGS_CHANNEL, log_text))
    except Exception as e:
